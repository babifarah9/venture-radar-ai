    ...
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-20 border-b border-white/8 bg-[#07110e]/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-5 lg:px-9">
          ...
          <Badge className="border-[#c7ff4a]/25 bg-[#c7ff4a]/10 text-[#d8ff7d]" variant="outline"><span className="size-1.5 rounded-full bg-[#c7ff4a]" /> {analysis?.mode === 'live' ? 'Live research' : 'Demo-ready'}</Badge>
        </div>
      </header>
...
function Report({ analysis: a }: { analysis: VentureAnalysis }) {
  return <section id="report" className="scroll-mt-20 mx-auto max-w-[1440px] px-5 py-8 lg:px-9">
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_1fr_.72fr]">
      ...
      <article className="rounded-2xl border bg-card p-6 shadow-sm"><p className="eyebrow">Domain shortlist</p><div className="mt-5 space-y-2">{a.domains.map((d) => <div key={d.domain} className="flex items-center justify-between rounded-xl bg-muted/60 px-3 py-3"><span className="font-mono text-xs font-semibold">{d.domain}</span><Badge variant={d.available ? 'default' : 'outline'} className={d.available ? 'bg-emerald-700' : ''}>{d.available === true ? 'Available' : d.available === false ? 'Taken' : 'Check'}</Badge></div>)}</div><p className="mt-3 text-[10px] leading-4 text-muted-foreground">Availability is confirmed through Name.com when credentials are configured; otherwise these are suggestions.</p></article>
